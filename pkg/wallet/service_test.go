...
func TestService_ExportAccountHistory_success(t *testing.T) {
	svc := newTestService()
	account, _, err := svc.addAccount(defaultTestAccount)
	if err != nil {
		t.Error(err)
	}
	_, err = svc.ExportAccountHistory(account.ID)
	if err != nil {
		t.Errorf("ExportAcccountHistory(): error=%v", err)
		return
	}

}
func TestService_HistoryToFile_success(t *testing.T) {
	svc := newTestService()
	account, _, err := svc.addAccount(defaultTestAccount)
	if err != nil {
		t.Error(err)
	}
	got, err := svc.ExportAccountHistory(account.ID)
	if err != nil {
		t.Errorf("ExportAcccountHistory(): error=%v", err)
		return
	}
	err = svc.HistoryToFile(got, "data", 2)
	if err != nil {
		t.Error(err)
	}
}
